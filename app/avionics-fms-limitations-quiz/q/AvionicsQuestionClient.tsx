...
export default function AvionicsQuestionClient() {
  ...
  const idx = Math.max(0, (parseInt(searchParams.get("n") ?? "", 10) || 1) - 1);
  ...
  function next() {
    ...
    if (idx + 1 >= total) {
      ...
      router.push(`/avionics-fms-limitations-quiz/q?n=${idx + 2}`);
    }
  }
  ...
  function prev() {
    ...
    if (idx > 0) {
      ...
      router.push(`/avionics-fms-limitations-quiz/q?n=${targetIdx + 1}`);
    }
  }
