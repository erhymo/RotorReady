...
import H145D2_ABBREVIATIONS from "@/data/h145d2/abbreviations";
...
const data = {
  ...
  mass: {
    ...
    items: [
      {
        label: "Weight",
        lines: ["Max gross mass: 3700 kg", "Min gross mass for flight: 2000 kg"],
      },
      {
        label: "Loading",
        lines: [
          ...
          "Forward cabin attachment / door frame lifeline: max 200 kg",
          "Multi-purpose brackets: max 200 kg/person (fwd/mid), max 10 kg equipment (aft)",
        ],
      },
    ],
  },
    ...
    items: [
      {
        label: "Rotor / N2",
        lines: [
          "NR min: 3% below nominal, or 94%, whichever is higher",
    ...
    items: [
      ...
      },
      {
        ...
        lines: [
          ...
          "FUEL RESERVE triggers: left supply tank <42 kg, right supply tank <48 kg",
        ],
      },
      {
        label: "Mast moment failure",
        lines: [
          "Avoid slope landings >3°",
          "Avoid crosswind >10 kt on takeoff/landing",
          "Center cyclic manually via stick position indicator",
        ],
      },
    ],
  },
...
export default function H145D2QuickReferencePage() {
  const [showAbbr, setShowAbbr] = useState(false);

  return (
    <div className="min-h-screen bg-slate-50 dark:bg-zinc-900">
      <AppTopBar
        title="H145 D2 Quick Reference"
        backHref="/"
        backLabel="Home"
        rightAction={
          <button
      ...
      <main className="mx-auto max-w-3xl p-4 space-y-4">
        <p className="text-xs text-slate-600 dark:text-zinc-300">
          For training use only. This is an informal quick reference of selected H145 D2 (BK117 D-2, Helionix
          SW V10) Flight Manual limitations. Always use the official Flight Manual as primary reference.
        </p>
        ...
        {showAbbr && (
          <section className="rounded-xl border bg-white dark:bg-zinc-900 dark:border-zinc-700 p-4 space-y-3">
            ...
            <ul className="divide-y divide-slate-200 dark:divide-zinc-700 rounded-xl border border-slate-200 dark:border-zinc-700 overflow-hidden">
              {H145D2_ABBREVIATIONS.map((row, i) => (
                <li
                  key={`${row.abbr}-${i}`}
