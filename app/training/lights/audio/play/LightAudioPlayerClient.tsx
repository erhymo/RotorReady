...
// Light id comes from `?lightId=` rather than a dynamic route segment, so this one
// bundled page serves every light — including ones recorded after the last native
  ...
  useEffect(() => {
    ...
    fetchContentJson<{ items?: LightAudioItem[] }>(`/audio/${activeVariant.id}/lights/index.json`)
      .then((data) => {
        ...
        setItem(items.find((entry) => entry.lightId === lightId) ?? null);
      })
      .catch(() => {
