...
const VARIANT_ID = "AW189" as const;
...
export default function Page() {
  ...
  // Ensure AW189 is active when landing directly
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        // Strict AW189-only filter and warnings only
        const filtered = merged.filter((item) => {
          ...
          if (modelIds && (modelIds.includes(VARIANT_ID) || modelIds.includes("AW189"))) return true;
          if (models && models.includes(VARIANT_ID)) return true;
          if (productIds && productIds.includes("AW189")) return true;
          if (typeof modelId === "string" && (modelId === VARIANT_ID || modelId === "AW189")) return true;
          if (typeof productId === "string" && productId === "AW189") return true;
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-zinc-900">
	      <AppTopBar title="CWP Trainer · AW189" backHref="/training/lights" backLabel="Lights" />
        ...
        <section className="rounded-xl border bg-white p-4 sm:p-6 shadow-sm dark:bg-zinc-900 dark:border-zinc-700">
          <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-xl font-semibold text-slate-900 dark:text-zinc-100">CWP-trainer · AW189</h1>
            </div>
