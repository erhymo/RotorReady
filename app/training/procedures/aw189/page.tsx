...
const GROUPS: { heading: string; slugs: string[] }[] = [
  {
    ...
    slugs: [
      "exterior-check",
      "cockpit-safety-checks-and-engine-starting",
      "taxiing-and-before-takeoff",
      "in-flight-and-before-landing",
      "shutdown",
    ],
  },
  {
    heading: "Engine failure",
    slugs: ["single-engine-failure", "double-engine-failure-autorotation", "engine-shutdown-and-ground-egress"],
  },
  {
    heading: "Fire",
    slugs: ["apu-and-engine-bay-fire", "cabin-and-baggage-fire", "electrical-fire-smoke"],
  },
  {
    heading: "Systems & controls",
    slugs: ["main-gearbox-and-rotor-controls", "tail-rotor-failures", "triple-ac-generator-failure", "hydraulic-pressure-low"],
  },
  {
    heading: "Category A operations (Supplement 4)",
    slugs: [
...
export default function AW189ProceduresListPage() {
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-zinc-900">
      <AppTopBar title="AW189 Procedures" backHref="/" backLabel="Home" />
      <main className="mx-auto max-w-3xl p-6 space-y-5">
        <h1 className="text-2xl font-bold text-slate-900 dark:text-zinc-100">AW189 Procedures</h1>
        <p className="text-sm text-slate-600 dark:text-zinc-300">
          Browse AW189 normal procedures, engine-failure response, fire procedures, and other emergency/malfunction
          procedures.
        </p>
        <AbbreviationsLinkCard href="/aw189/abbreviations" />
        {GROUPS.map((group) => (
          <section key={group.heading} className="space-y-3">
            ...
            <div className="rounded-xl border bg-white dark:bg-zinc-800 dark:border-zinc-700 p-4 space-y-3">
              {group.slugs.map((slug) => {
                const proc = AW189_PROCEDURES.find((p) => p.slug === slug);
                if (!proc) return null;
                return (
                  <Link
                    key={slug}
                    href={`/aw189/procedures/${slug}`}
