...
import { H145D2_PROCEDURES } from "@/lib/procedures/h145-d2/data";
...
export default function H145D2ProceduresListPage() {
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-zinc-900">
      <AppTopBar title="H145 D2 Procedures" backHref="/" backLabel="Home" />
      <main className="mx-auto max-w-3xl p-6 space-y-5">
        <h1 className="text-2xl font-bold text-slate-900 dark:text-zinc-100">H145 D2 Procedures</h1>
        <p className="text-sm text-slate-600 dark:text-zinc-300">
          Browse H145 D2 normal procedures — exterior check, engine start, takeoff, landing and IFR approaches —
          plus engine emergency, fire, drive-system and fuel procedures.
        </p>
        <AbbreviationsLinkCard href="/h145-d2/abbreviations" />
        {GROUPS.map((group) => (
          <section key={group.heading} className="space-y-3">
            ...
            <div className="rounded-xl border bg-white dark:bg-zinc-800 dark:border-zinc-700 p-4 space-y-3">
              {group.slugs.map((slug) => {
                const proc = H145D2_PROCEDURES.find((p) => p.slug === slug);
                if (!proc) return null;
                return (
                  <Link
                    key={slug}
                    href={`/h145-d2/procedures/${slug}`}
