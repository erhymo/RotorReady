...
// "(loading…)" used to be shown for three different things: still loading, the request
// failed, and the airport simply has no METAR/TAF. With no signal (hangar, in flight) a
// failed request therefore looked like a page that never finished loading.
function wxPlaceholder(wx: Wx | undefined): string {
  if (!wx) return "(loading…)";
  return wx.failed ? "(could not load — check your connection)" : "(not available)";
}
...
export default function WeatherDetail() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const entries = await Promise.all(
        targets.map(async (a) => {
          try {
            const res = await fetch(apiUrl(`/api/weather/metar-taf?icao=${a.icao}`), { cache: "no-store" });
            ...
            return [a.icao, { icao: a.icao, failed: true } as Wx] as const;
          }
        })
      );
      if (active) {
  ...
  useEffect(() => {
    ...
    async function loadDayNight() {
      ...
      try {
        const res = await fetch(apiUrl(`/api/weather/daynight?icao=${airport.icao}`), { cache: "no-store" });
        if (!res.ok) {
          if (active) setDayNight(null);
          return;
        }
              ...
              return (
                <>
                  ...
                  <div className="text-sm text-slate-700 dark:text-zinc-200">
                    <span className="font-medium">METAR:</span> {wx?.metarRaw || <span className="text-slate-400">{wxPlaceholder(wx)}</span>}
                  </div>
                  {(() => {
                  ...
                  <div className="text-sm text-slate-700 dark:text-zinc-200">
                    ...
                    {wx?.tafChunks && wx.tafChunks.length > 0 ? (
                      ...
                      <span className="text-slate-400"> {wx?.tafRaw || wxPlaceholder(wx)}</span>
                    )}
                  </div>
